(function () {
  ...
  const forceEvents = new Set(["playBomb", "playJokerBomb", "win"]);
  const lastTalkAt = new Map();

  const personalities = [
    {
      id: "liubei", name: "刘备", title: "昭烈仁君", faction: "蜀",
      personality: "仁义稳重，重视同伴，不轻易冒进。",
      avatar: "./assets/skins/sanguo/avatars/characters/liubei.png", fullBody: "./assets/skins/sanguo/portraits/liubei.png", seat: "left", facing: "rightDown", camp: "shu", riskLevel: 35,
      lines: {
    ...
    {
      id: "guanyu", name: "关羽", title: "武圣云长", faction: "蜀",
      personality: "骄傲强势，出手果断，压制感强。",
      avatar: "./assets/skins/sanguo/avatars/characters/guanyu.png", fullBody: "./assets/skins/sanguo/portraits/guanyu.png", seat: "left", facing: "rightDown", camp: "shu", riskLevel: 72,
      lines: {
    ...
    {
      id: "zhangfei", name: "张飞", title: "燕人翼德", faction: "蜀",
      personality: "暴躁豪爽，喜欢强攻，气势压人。",
      avatar: "./assets/skins/sanguo/avatars/characters/zhangfei.png", fullBody: "./assets/skins/sanguo/portraits/zhangfei.png", seat: "left", facing: "rightDown", camp: "shu", riskLevel: 88,
      lines: {
    ...
    {
      id: "zhugeliang", name: "诸葛亮", title: "卧龙军师", faction: "蜀",
      personality: "冷静谋略，善于观察节奏和牌势。",
      avatar: "./assets/skins/sanguo/avatars/characters/zhugeliang.png", fullBody: "./assets/skins/sanguo/portraits/zhugeliang.png", seat: "left", facing: "rightDown", camp: "shu", riskLevel: 48,
      lines: {
    ...
    {
      id: "caocao", name: "曹操", title: "魏武孟德", faction: "魏",
      personality: "多疑霸气，敢压敢控，喜欢掌握主动。",
      avatar: "./assets/skins/sanguo/avatars/characters/caocao.png", fullBody: "./assets/skins/sanguo/portraits/caocao.png", seat: "left", facing: "rightDown", camp: "wei", riskLevel: 78,
      lines: {
    ...
    {
      id: "simayi", name: "司马懿", title: "冢虎仲达", faction: "晋",
      personality: "隐忍阴狠，少言冷静，等待致命机会。",
      avatar: "./assets/skins/sanguo/avatars/characters/simayi.png", fullBody: "./assets/skins/sanguo/portraits/simayi.png", seat: "left", facing: "rightDown", camp: "neutral", riskLevel: 62,
      lines: {
  ...
  const byId = Object.fromEntries(personalities.map((item) => [item.id, item]));
  ...
  function getCharacterForPlayer(playerId) {
    if (!playerId) return personalities[0];
    return personalities[hashText(playerId) % personalities.length];
  }

  function getLine(characterId, eventName, options) {
    const character = byId[characterId];
    const lines = character?.lines?.[eventName];
    if (!lines || lines.length === 0) return "";
    const force = Boolean(options?.force) || forceEvents.has(eventName);
    const now = Date.now();
    const last = lastTalkAt.get(characterId) || 0;
    if (!force && now - last < TALK_COOLDOWN_MS) return "";
    lastTalkAt.set(characterId, now);
  ...
  window.ddzDialogue = {
    ...
    getLine,
    resetCooldown(characterId) {
      if (characterId) lastTalkAt.delete(characterId);
      else lastTalkAt.clear();
    },
    forceEvents,
  };
})();
